...
```
The configuration is defined in `.php-cs-fixer.dist.php` using the `@PER-CS` and `@PHP8x1Migration` rulesets.
...
- `Jmonitor\JmonitorBundle\Collector\SymfonyCollector`: Collects Symfony-specific metrics (bundles, environment, version, directories). Delegates to component collectors via `tagged_iterator`.
- `Jmonitor\JmonitorBundle\Collector\Components\SchedulerCollector`: Parses `debug:scheduler` output to collect scheduled tasks.
- `Jmonitor\JmonitorBundle\Collector\Components\FlexRecipesCollector`: Detects outdated Flex recipes via `composer recipes`.
- `Jmonitor\JmonitorBundle\Collector\Components\MessengerStatsCollector`: Collects queue statistics via `messenger:stats --format=json`.
- `Jmonitor\JmonitorBundle\Collector\CommandRunner`: Utility to run Symfony console commands in-process or as external processes with timeout support.
