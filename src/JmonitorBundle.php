        ...
        $definition->rootNode()
            ->children() // jmonitor
                ...
                ->arrayNode('schedule')
                    ->canBeEnabled()
                    ->children()
                        ->scalarNode('name')->isRequired()->cannotBeEmpty()->info('Name of the schedule used to handle the recurring metrics collection.')->end()
                        ->integerNode('frequency')->isRequired()->info('Frequency in seconds. Set it to 30 if you\'re not sure (free plan).')->end()
                    ->end()
            ...
            ->end()
            ->validate()
            ->ifTrue(static fn($config): bool => !empty($config['schedule']['name']) && !class_exists(Scheduler::class))
