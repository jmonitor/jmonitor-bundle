        ...
        $definition->rootNode()
            ->children() // jmonitor
                ...
                ->arrayNode('schedule')
                    ->canBeEnabled()
                    ->children()
                        ->scalarNode('name')->defaultValue('default')->cannotBeEmpty()->info('Name of the schedule used to handle the recurring metrics collection.')->end()
                        ->integerNode('frequency')->defaultValue(30)->info('Frequency in seconds. Set it to 30 if you\'re not sure (free plan).')->end()
                    ->end()
                    ->info('Must be set to enable use of symfony scheduler.')
                ->end()
            ...
            ->end()
            ->validate()
            ->ifTrue(static fn($config): bool => $config['schedule']['enabled'] && !class_exists(Scheduler::class))
